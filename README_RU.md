...
- Поддержка нескольких терминальных приложений: iTerm, Terminal, Warp.
...
1. **Редактирование конфигурации**: Щелкните правой кнопкой мыши по значку в трее и выберите "Edit Config", чтобы открыть файл конфигурации в вашем редакторе по умолчанию. Измените конфигурацию по мере необходимости.
2. **Открытие папки конфигурации**: Щелкните правой кнопкой мыши по значку в трее и выберите "Open Config Folder

", чтобы открыть каталог конфигурации в вашем файловом менеджере.
3. **Переключение запуска при входе в систему**: Щелкните правой кнопкой мыши по значку в трее и выберите "Toggle Launch at Login", чтобы включить или отключить автоматический запуск приложения при входе в систему.
4. **Выполнение команды**: Щелкните левой кнопкой мыши по значку в трее и выберите команду, которую хотите выполнить из меню. Команда будет выполнена в указанном терминальном приложении.

### Создание подменю

Чтобы создать подменю, установите поле `command` в `null` и предоставьте список подкоманд в поле `submenu`. Подкоманды также могут иметь свои собственные подменю, что позволяет создавать вложенные меню.
