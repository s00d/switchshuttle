...
Мониторинг системных ресурсов и сервисов с информацией в реальном времени:
...
- **Интеграция с меню** - Добавление кнопок мониторинга в системное меню
- **Выполнение команд** - Выполнение команд мониторинга при открытии меню
- **Отображение данных** - Показ результата команды прямо в интерфейсе меню
- **Визуальные индикаторы** - Иконки и индикаторы статуса в меню
- **Кроссплатформенность** - Работает на macOS, Windows и Linux

#### ⏰ Запланированные команды (Cron)

Запланируйте команды для автоматического выполнения с помощью cron выражений:

```json
{
  "name": "🔄 Авто резервное копирование",
  "commands": [
...
- **Cron выражения** - Используйте стандартный cron формат для планирования
- **Фоновое выполнение** - Запуск запланированных команд в тишине
- **Кроссплатформенность** - Работает на macOS, Windows и Linux
- **Постоянство** - Расписания продолжают работать даже когда меню закрыто

#### 🖥️ Фоновое выполнение

Контролируйте, как выполняются команды - в фоне через ConsolePool или обычное выполнение в терминале:
...
**Опции фонового выполнения:**
- `"background": true` - Выполнение через ConsolePool (в фоне)
- `"background": false` - Выполнение через обычный терминал
...
| `monitor` | String | ❌ | Команда для получения отображаемого значения мониторинга |
...
| `background` | Boolean | ❌ | Выполнять в фоне (ConsolePool) или обычный терминал |
