<template>
  <div class="space-y-4">
    <div class="flex items-center justify-between">
      <label class="block text-sm font-semibold text-slate-700">Scheduler</label>
      <div class="flex items-center space-x-2">
        <Button 
          @click="toggleEditMode" 
          variant="ghost" 
          size="sm"
          :class="editMode ? 'text-blue-600 hover:text-blue-700 hover:bg-blue-50' : 'text-slate-600 hover:text-slate-700 hover:bg-slate-50'"
        >
        ...
        <Button 
          ...
          variant="ghost" 
          size="sm" 
          :class="schedulerEnabled ? 'text-green-600 hover:text-green-700 hover:bg-green-50' : 'text-slate-600 hover:text-slate-700 hover:bg-slate-50'"
        >
          <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div v-if="schedulerEnabled" class="space-y-4 p-4 bg-slate-50 rounded-lg">
      ...
      <div v-if="editMode" class="space-y-4">
        ...
        <div class="bg-blue-50 p-3 rounded-lg">
          <h4 class="text-sm font-semibold text-blue-900 mb-2">Cron Expression Format</h4>
      ...
      <div v-else class="space-y-4">
        <div class="grid grid-cols-6 gap-1">
          <!-- Second -->
                      <div class="space-y-1">
              <label class="block text-xs font-medium text-slate-700">Second</label>
                          <SchedulerSelect
              v-model="cronFields.second"
              :options="secondOptions"
              placeholder="0-59"
              @update:modelValue="(value) => { cronFields.second = value; updateCronExpression(); }"
            />
          </div>

          <!-- Minute -->
          <div class="space-y-1">
            <label class="block text-xs font-medium text-slate-700">Minute</label>
            <SchedulerSelect
              v-model="cronFields.minute"
              :options="minuteOptions"
              placeholder="0-59"
              @update:modelValue="(value) => { cronFields.minute = value; updateCronExpression(); }"
            />
          </div>
          ...
          <div class="space-y-1">
            ...
            <SchedulerSelect
              ...
              :options="hourOptions"
              placeholder="0-23"
              @update:modelValue="(value) => { cronFields.hour = value; updateCronExpression(); }"
            />
          </div>
          ...
          <div class="space-y-1">
            ...
            <SchedulerSelect
              ...
              :options="dayOptions"
              placeholder="1-31"
              @update:modelValue="(value) => { cronFields.day = value; updateCronExpression(); }"
            />
          </div>

          <!-- Month -->
          <div class="space-y-1">
            <label class="block text-xs font-medium text-slate-700">Month</label>
            <SchedulerSelect
              v-model="cronFields.month"
              :options="monthOptions"
              placeholder="1-12"
              @update:modelValue="(value) => { cronFields.month = value; updateCronExpression(); }"
            />
          </div>
          ...
          <div class="space-y-1">
            ...
            <SchedulerSelect
              ...
              placeholder="0-6"
              @update:modelValue="(value) => { cronFields.weekday = value; updateCronExpression(); }"
            />
          </div>
        </div>
        ...
        <!-- Preset Buttons -->
        <div class="space-y-2">
          <label class="block text-xs font-medium text-slate-700">Quick Presets</label>
        ...
        <div class="bg-white p-3 rounded border">
          <label class="block text-xs font-medium text-slate-700 mb-1">Current Expression</label>
          <code class="text-sm bg-slate-100 px-2 py-1 rounded">{{ cronExpression }}</code>
        </div>
      </div>
    </div>
  </div>
...
const schedulerEnabled = computed(() => cronExpression.value && cronExpression.value.trim() !== '');
...
const updateScheduler = () => {
  emit('update:modelValue', schedulerEnabled.value ? cronExpression.value : undefined);
};
