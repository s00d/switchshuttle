...
use crate::helpers::{get_config_path, open_in_default_editor, open_folder_in_default_explorer};
...
pub fn get_terminals_list() -> HashMap<String, TerminalConfig> {
    ...
    terminals.into_iter().map(|(key, config)| {
...
pub fn save_or_update_configuration(
    ...
    if let Err(e) = hotkey_manager.register_all_hotkeys(&config_manager.configs, &app, &settings_state) {
